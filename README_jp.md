...
- `cornix_indicator`: battery / connection status 用 RGB LED indicator shield。消費電力が増えるため注意してください。
...
Cornix には各 side に RGB LED がありますが、このリポジトリでは stock firmware と同等の RGB indicator はまだ完全対応ではありません。
将来的には battery status や connection status 表示用の RGB indicator module へ適応する予定です。
